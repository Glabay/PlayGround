...
public class Screen {
	...
	public void blendTiles(int xp, int yp, Tile tile1, Tile tile2) {
		...
		for (int y = 0; y < tile1.sprite.SIZE; y++) {
			...
			for (int x = 0; x < tile1.sprite.SIZE; x++) {
				int xa = x + xp;
				if (xa < -tile1.sprite.SIZE || xa >= width || ya < 0 || ya >= height)
					break;
				if (xa < 0)
					xa = 0;
	...
	public void blendSprite(int xp, int yp, Sprite s1, Sprite s2) {
		...
		for (int y = 0; y < s1.SIZE; y++) {
			...
			for (int x = 0; x < s1.SIZE; x++) {
				int xa = x + xp;
				if (xa < -s1.SIZE || xa >= width || ya < 0 || ya >= height)
					break;
				if (xa < 0)
					xa = 0;
				type = random.nextInt(2 + y);
	...
	public void renderTile(int xp, int yp, Tile tile) {
		...
		for (int y = 0; y < tile.sprite.SIZE; y++) {
			...
			for (int x = 0; x < tile.sprite.SIZE; x++) {
				...
				/*
				 * if (col == 0xff1F1F1F) { col += 0xff00FF00; }
				 */
				if (col != ALPHA_COL) {
					col = colSwitch(col, tilesx, tilesy);
					pixels[xa + ya * width] = col;
	...
	/*
	 * public void renderTile(int xp, int yp, Tile tile, int col) { int tilesx = xp; int tilesy = yp; xp -= xOffset; yp -= yOffset; for (int y = 0; y <tile.sprite.SIZE; y++) { int ya = y + yp; for (int x = 0; x <tile.sprite.SIZE; x++) { int xa = x + xp; if (xa < -tile.sprite.SIZE || xa >= width || ya < 0 || ya >= height) break; if (xa < 0) xa = 0; //New way for Day / Night Cycles col = tile.sprite.pixels[x + y * tile.sprite.SIZE]; /* if (col == 0xff1F1F1F) { col += 0xff00FF00; } if (col != ALPHA_COL) { col = colSwitch(col, tilesx, tilesy); pixels[xa + ya * width] = col; } //-------------------------- // pixels[xa + ya * width] = tile.sprite.pixels[x + y * tile.sprite.SIZE]; } } }
	 */
	...
	private int test(int tilesx, int tilesy) {
		...
		if (bool) {
			...
			if (Game.level.getOverlayTile((tilesx >> Game.TILE_BIT_SHIFT), (tilesy >> Game.TILE_BIT_SHIFT)) instanceof Pebble)
				return 1;
			if (Game.level.getOverlayTile((tilesx >> Game.TILE_BIT_SHIFT) + 0, (tilesy >> Game.TILE_BIT_SHIFT) + 1) instanceof Pebble)
				return 1;
			if (Game.level.getOverlayTile((tilesx >> Game.TILE_BIT_SHIFT) + 0, (tilesy >> Game.TILE_BIT_SHIFT) - 1) instanceof Pebble)
				return 1;
			if (Game.level.getOverlayTile((tilesx >> Game.TILE_BIT_SHIFT) + 1, (tilesy >> Game.TILE_BIT_SHIFT) + 0) instanceof Pebble)
				return 1;
			if (Game.level.getOverlayTile((tilesx >> Game.TILE_BIT_SHIFT) - 1, (tilesy >> Game.TILE_BIT_SHIFT) + 0) instanceof Pebble)
				return 1;

			/*
			 * if (level.getTile((tilesx >> Game.TILE_BIT_SHIFT) + 0, (tilesy >> Game.TILE_BIT_SHIFT) + 1).illuminator()) return 1; if (level.getTile((tilesx >> Game.TILE_BIT_SHIFT) + 0, (tilesy >> Game.TILE_BIT_SHIFT) - 1).illuminator()) return 1; if (level.getTile((tilesx >> Game.TILE_BIT_SHIFT) + 1, (tilesy >> Game.TILE_BIT_SHIFT) + 0).illuminator()) return 1; if (level.getTile((tilesx >> Game.TILE_BIT_SHIFT) - 1, (tilesy >> Game.TILE_BIT_SHIFT) + 0).illuminator()) return 1;
			 */

			if (Game.level.getOverlayTile((tilesx >> Game.TILE_BIT_SHIFT) + 1, (tilesy >> Game.TILE_BIT_SHIFT) + 1) instanceof Pebble)
				return 2;
			if (Game.level.getOverlayTile((tilesx >> Game.TILE_BIT_SHIFT) + 1, (tilesy >> Game.TILE_BIT_SHIFT) - 1) instanceof Pebble)
				return 2;
			if (Game.level.getOverlayTile((tilesx >> Game.TILE_BIT_SHIFT) - 1, (tilesy >> Game.TILE_BIT_SHIFT) + 1) instanceof Pebble)
			...
			/*
			 * if (level.getTile((tilesx >> Game.TILE_BIT_SHIFT) + 1, (tilesy >> Game.TILE_BIT_SHIFT) + 1).illuminator()) return 2; if (level.getTile((tilesx >> Game.TILE_BIT_SHIFT) + 1, (tilesy >> Game.TILE_BIT_SHIFT) - 1).illuminator()) return 2; if (level.getTile((tilesx >> Game.TILE_BIT_SHIFT) - 1, (tilesy >> Game.TILE_BIT_SHIFT) + 1).illuminator()) return 2; if (level.getTile((tilesx >> Game.TILE_BIT_SHIFT) - 1, (tilesy >> Game.TILE_BIT_SHIFT) - 1).illuminator()) return 2;
			 */

			if (Game.level.getOverlayTile((tilesx >> Game.TILE_BIT_SHIFT) + 2, (tilesy >> Game.TILE_BIT_SHIFT) + 0) instanceof Pebble)
				return 3;
			if (Game.level.getOverlayTile((tilesx >> Game.TILE_BIT_SHIFT) - 2, (tilesy >> Game.TILE_BIT_SHIFT) + 0) instanceof Pebble)
				return 3;
			if (Game.level.getOverlayTile((tilesx >> Game.TILE_BIT_SHIFT) + 0, (tilesy >> Game.TILE_BIT_SHIFT) + 2) instanceof Pebble)
				return 3;
			if (Game.level.getOverlayTile((tilesx >> Game.TILE_BIT_SHIFT) + 0, (tilesy >> Game.TILE_BIT_SHIFT) - 2) instanceof Pebble)
				return 3;
			if (Game.level.getOverlayTile((tilesx >> Game.TILE_BIT_SHIFT) + 1, (tilesy >> Game.TILE_BIT_SHIFT) + 2) instanceof Pebble)
				return 3;
			if (Game.level.getOverlayTile((tilesx >> Game.TILE_BIT_SHIFT) + 1, (tilesy >> Game.TILE_BIT_SHIFT) - 2) instanceof Pebble)
				return 3;
			if (Game.level.getOverlayTile((tilesx >> Game.TILE_BIT_SHIFT) - 1, (tilesy >> Game.TILE_BIT_SHIFT) + 2) instanceof Pebble)
			...
			/*
			 * if (level.getTile((tilesx >> Game.TILE_BIT_SHIFT) + 2, (tilesy >> Game.TILE_BIT_SHIFT) + 0).illuminator()) return 3; if (level.getTile((tilesx >> Game.TILE_BIT_SHIFT) - 2, (tilesy >> Game.TILE_BIT_SHIFT) + 0).illuminator()) return 3; if (level.getTile((tilesx >> Game.TILE_BIT_SHIFT) + 0, (tilesy >> Game.TILE_BIT_SHIFT) + 2).illuminator()) return 3; if (level.getTile((tilesx >> Game.TILE_BIT_SHIFT) + 0, (tilesy >> Game.TILE_BIT_SHIFT) - 2).illuminator()) return 3; if (level.getTile((tilesx >> Game.TILE_BIT_SHIFT) + 1, (tilesy >> Game.TILE_BIT_SHIFT) + 2).illuminator()) return 3; if (level.getTile((tilesx >> Game.TILE_BIT_SHIFT) + 1, (tilesy >> Game.TILE_BIT_SHIFT) - 2).illuminator()) return 3; if (level.getTile((tilesx >> Game.TILE_BIT_SHIFT) - 1, (tilesy >> Game.TILE_BIT_SHIFT) + 2).illuminator()) return 3; if (level.getTile((tilesx >> Game.TILE_BIT_SHIFT) - 1, (tilesy >> Game.TILE_BIT_SHIFT) - 2).illuminator()) return 3;
			 */

			if (Game.level.getOverlayTile((tilesx >> Game.TILE_BIT_SHIFT) + 2, (tilesy >> Game.TILE_BIT_SHIFT) + 1) instanceof Pebble)
				return 4;
			if (Game.level.getOverlayTile((tilesx >> Game.TILE_BIT_SHIFT) + 2, (tilesy >> Game.TILE_BIT_SHIFT) - 1) instanceof Pebble)
				return 4;
			if (Game.level.getOverlayTile((tilesx >> Game.TILE_BIT_SHIFT) - 2, (tilesy >> Game.TILE_BIT_SHIFT) + 1) instanceof Pebble)
		...
		/*
		 * if (level.getTile((tilesx >> Game.TILE_BIT_SHIFT) + 2, (tilesy >> Game.TILE_BIT_SHIFT) + 1).illuminator()) return 4; if (level.getTile((tilesx >> Game.TILE_BIT_SHIFT) + 2, (tilesy >> Game.TILE_BIT_SHIFT) - 1).illuminator()) return 4; if (level.getTile((tilesx >> Game.TILE_BIT_SHIFT) - 2, (tilesy >> Game.TILE_BIT_SHIFT) + 1).illuminator()) return 4; if (level.getTile((tilesx >> Game.TILE_BIT_SHIFT) - 2, (tilesy >> Game.TILE_BIT_SHIFT) - 1).illuminator()) return 4;
		 */
		return 0;
	}
	...
	private void deterLightLevel(int tilesx, int tilesy) {
		...
		if (bool) {
			...
			if (level.getTile((tilesx), (tilesy)).solid()) {
				lightlevels[tilesx][tilesy] = 0;
				return;
			}
			if (level.getOverlayTile((tilesx), (tilesy)) instanceof TorchTile) {
				lightlevels[tilesx][tilesy] = 1;
				return;
			}

			if (level.getOverlayTile((tilesx) + 0, (tilesy) + 1) instanceof TorchTile) {
				lightlevels[tilesx][tilesy] = 1;
				return;
			}
			if (level.getOverlayTile((tilesx) + 0, (tilesy) - 1) instanceof TorchTile) {
			...
			if (level.getOverlayTile((tilesx) + 1, (tilesy) + 0) instanceof TorchTile) {
				lightlevels[tilesx][tilesy] = 1;
				return;
			}
			if (level.getOverlayTile((tilesx) - 1, (tilesy) + 0) instanceof TorchTile) {
			...
			if (level.getOverlayTile((tilesx) + 1, (tilesy) + 1) instanceof TorchTile) {
				lightlevels[tilesx][tilesy] = 2;
				return;
			}
			if (level.getOverlayTile((tilesx) + 1, (tilesy) - 1) instanceof TorchTile) {
				lightlevels[tilesx][tilesy] = 2;
			...
			if (level.getOverlayTile((tilesx) + 2, (tilesy) + 0) instanceof TorchTile) {
				lightlevels[tilesx][tilesy] = 3;
				return;
			}
			if (level.getOverlayTile((tilesx) - 2, (tilesy) + 0) instanceof TorchTile) {
				lightlevels[tilesx][tilesy] = 3;
			...
			if (level.getOverlayTile((tilesx) + 0, (tilesy) + 2) instanceof TorchTile) {
				lightlevels[tilesx][tilesy] = 3;
				return;
			}
			if (level.getOverlayTile((tilesx) + 0, (tilesy) - 2) instanceof TorchTile) {
			...
			if (level.getOverlayTile((tilesx) + 3, (tilesy) + 0) instanceof TorchTile) {
				lightlevels[tilesx][tilesy] = 5;
				return;
			}
			if (level.getOverlayTile((tilesx) - 3, (tilesy) + 0) instanceof TorchTile) {
				lightlevels[tilesx][tilesy] = 5;
			...
			if (level.getOverlayTile((tilesx) + 0, (tilesy) + 3) instanceof TorchTile) {
				lightlevels[tilesx][tilesy] = 5;
				return;
			}
			if (level.getOverlayTile((tilesx) + 0, (tilesy) - 3) instanceof TorchTile) {
			...
			if (level.getOverlayTile((tilesx) + 2, (tilesy) - 1) instanceof TorchTile) {
				lightlevels[tilesx][tilesy] = 4;
				return;
			}
			if (level.getOverlayTile((tilesx) + 2, (tilesy) + 1) instanceof TorchTile) {
				lightlevels[tilesx][tilesy] = 4;
			...
			if (level.getOverlayTile((tilesx) + 1, (tilesy) + 2) instanceof TorchTile) {
				lightlevels[tilesx][tilesy] = 4;
				return;
			}
			if (level.getOverlayTile((tilesx) - 1, (tilesy) + 2) instanceof TorchTile) {
	...
	public void renderLight(int xp, int yp, int radius, Mob mob, int r, int g, int b) {
		...
		for (int y = 0; y < radius * 2; y++) {
			...
			for (int x = 0; x < radius * 2; x++) {
				...
				if (d < radius) {
					int d2;
					if (mob.dir == mob.dir.LEFT)
						d2 = (int) (((y - radius) * (y - radius)) / (10)) - ((x - radius) * -4);
					else if (mob.dir == mob.dir.RIGHT)
						d2 = (int) (((y - radius) * (y - radius)) / (10)) - ((x - radius) * 4);
					else if (mob.dir == mob.dir.UP)
						d2 = (int) (((x - radius) * (x - radius)) / (10)) - ((y - radius) * -4);
					else if (mob.dir == mob.dir.DOWN)
						d2 = (int) (((x - radius) * (x - radius)) / (10)) - ((y - radius) * 4);
					else if (mob.dir == mob.dir.UP)
						d2 = (int) (((x - radius) * (x - radius)) / (10)) - ((y - radius) * -4);
					else if (mob.dir == mob.dir.DOWN)
						d2 = (int) (((x - radius) * (x - radius)) / (10)) - ((y - radius) * -4);
					else if (mob.dir == mob.dir.LEFT)
						d2 = (int) (((x - radius) * (x - radius)) / (10)) - ((y - radius) * -4);
					else if (mob.dir == mob.dir.RIGHT)
						d2 = (int) (((x - radius) * (x - radius)) / (10)) - ((y - radius) * 4);
					else
		...
		try {
			...
			for (int y = 0; y < map.SIZE; y++) {
				...
				for (int x = 0; x < map.SIZE; x++) {
					 ...
					 * try {
					 * 
					 * for(int i = 0; i < e.size(); i++) { if (!(e.get(i) instanceof Player)) { ex = (((int)e.get(i).getX() >> Game.TILE_BIT_SHIFT) - renderXt); ey = (((int)e.get(i).getY() >> Game.TILE_BIT_SHIFT) - renderYt); } } if (!(ex < renderX && ey < renderY && ex > renderX + size && ey > renderY + size)) { System.out.println("TRUE"); map.pixels[ex + ey * map.getWidth()] = 0xff000000; } } catch (Exception err) {
					 * 
					 * }
					 */
					if (size == 45) {
	...
	/*
	 * SpriteSheet sheet = SpriteSheet.minimapDYN; for (int y = 0; y < (int)(Game.getGame().getPlayer().getY() / 16) + 32; y++) { int ya = y + yp; for (int x = 0; x < (int)(Game.getGame().getPlayer().getX() / 16) + 32; x++) { int xa = x + xp; if (xa < 0 || xa >= width || ya < 0 || ya >= height) continue; pixels[xa + ya * width] = sheet.pixels[x + y * 32]; int color = sheet.pixels[x + y * 32]; if (color != ALPHA_COL) { pixels[xa + ya * width] = color; } } }//(int)(Game.getGame().getPlayer().getY() / 16) + }
	 */

	/*
	 * public int colSwitch(int col) { col = MyColor.changeBrightness(col, Level.brightness, false); return col; }
	 */

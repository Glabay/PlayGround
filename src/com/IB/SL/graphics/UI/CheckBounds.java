...
public class CheckBounds extends UI implements Serializable {
		...
		if (!Game.getGame().gui.charMenu) {
			...
			if (checkBounds(252, 140, 37, 16, true, true)) {
				if (Mouse.getButton() == 1) {
					...
					/*
					 * if (Game.getGame().runTut) { Game.getGame().getPlayer().setPosition(71, 12, Maps.tutWorldId, true); }
					 */
					Sound.Play(Sound.Click, false);
					Mouse.setMouseB(-1);
					Game.getGame().gui.charMenu = false;
	...
	public void checkDeath() {
		if (Game.getGame().gameState == Game.getGame().gameState.DEATH) {
			/*
			 * if (Mouse.getX() < 85 + 300 && Mouse.getX() > 85 - 5 && Mouse.getY() < 270 + 105 && Mouse.getY() > 270 - 10) {
			 */
			...
			/*
			 * if (Mouse.getX() < 808 + 237 && Mouse.getX() > 808 - 4 && Mouse.getY() < 268 + 107 && Mouse.getY() > 268 - 8) {
			 */
